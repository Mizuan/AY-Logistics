...
$(function () {
    ...
    $("#ManifestPreAlertTable").addClass("hide");
    $("#ManifestPreAlertTable_wrapper").addClass("hide");
...
function DashBoard() {
    ...
    self.init = function () {
        $.getJSON("../Manifest/GetDashboardItem", function (data) {
            self.dashboarditem(data);
        })
    }
    self.FilterStatus = function (data) {
        bcrump.breadcrumptlist.removeAll();
        bcrump.breadcrumptlist.push(new breadItem("Dash Board", true, 1, data.ShipmentStatusId));
        bcrump.breadcrumptlist.push(new breadItem(data.Name, false, 2, data.ShipmentStatusId));
        getFilterStatus(data.ShipmentStatusId);
    }
}
...
function breadItem(name, divider, level, ShipmentStatusId) {
    var self = this;
    ...
    self.navigate = function () {
        ...
        if (level == 2) {
         ...
         //   $("#dashboard").removeClass("hidden");
            dash.init();
            getFilterStatus(ShipmentStatusId)
        }
        if (level == 1) {
            window.location = '../Manifest/index';
        }
...
function getFilterStatus(status) {
 //   alert(status);
    ...
    else if (status == -1)
    {
        ...
        $("#ManifestTable").addClass("hide");
        $("#ManifestTable_wrapper").addClass("hide");
        $("#ManifestPreAlertTable").removeClass("hide");
        $('#ManifestPreAlertTable').css('width', '100%');
        ManifestPreAlertTable(status);
    }
    else
    {
        $("#BOLTable").addClass("hide");
