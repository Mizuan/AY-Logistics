...
namespace AYLogistics.Models
{
    public class ManifestModel
    {
        ...
        {
            string Format = MySettings.FormatNumbering(6);// GetHBLNumberFormat();
            string CompanyCode = "";
            string HBLNumber ="";
            if (DOAId == 1017)
            {
            ...
            HBLNumber = Format.Replace("CODE", CompanyCode);
       ...
       /* public static int GetHBLSequence()
        ...
        {
            string FormatJobNumber = "";
            if (MS == 1)
            {
                FormatJobNumber = GetJobSEANumberFormat(); //MySettings.FormatNumbering(2);
            }
            if (MS == 2)
            {
                FormatJobNumber = GetJobAIRNumberFormat(); //MySettings.FormatNumbering(3);
            }
            if (MS == 3)
            {
                FormatJobNumber = GetJobAIRNumberFormat(); //MySettings.FormatNumbering(4);
            }
            //return FormatJobNumber.Replace("%S%", GetJobSequence().ToString()).Replace("%YYYY%", Convert.ToString(DateTime.Today.Year));
           // return FormatJobNumber;
            return FormatJobNumber.Replace("%S%", MySettings.GetSingleSquence(2)).Replace("%YYYY%", Convert.ToString(DateTime.Today.Year));
        }
        ...
        public string PreSaveManifest()
        {
            string ManifestNumber = MySettings.FormatNumbering(1);//FormatManifestNumber();
           ...
           DBAccess.Insert(sql, List, ConnectionString.DEFAULT);
...
,[DeliveryAgentId] = @DeliveryAgentId
        ...
        public static int GetManifestId(string Number)
        {
            string sql = "select Id from Manifest where Number = @Number";
            List<SqlParameter> list = new List<SqlParameter>();
            SqlParameter param = new SqlParameter();
            param = new SqlParameter("@Number", DbType.Int32) { Value = Number };
            list.Add(param);
            SqlDataReader reader = DBAccess.FetchResult(sql, list, ConnectionString.DEFAULT);
            int ManifestId = 0;
            while (reader.Read())
            {
                ManifestId = reader["Id"] == DBNull.Value ? 0 : (Convert.ToInt32(reader["Id"])); // DBNull.Value Int
            }
            reader.Close();
...
,@CMeasure
          ...
          param = new SqlParameter("@CWeight", DbType.Decimal);
          param.Value = HouseBLItems[j].ContainerItems[i].CWeight;
          List.Add(param);
          param = new SqlParameter("@CMeasure", DbType.Decimal);
          param.Value = HouseBLItems[j].ContainerItems[i].CMeasure;
          List.Add(param);
...
,[CMeasure]
 ...
 ,@CMeasure
       ...
       /* public bool UpdateSequence()
        ...
        public bool UpdateSequence()
        {
            String sql = @"UPDATE [dbo].[Numbering]
                                ...
                                WHERE Id= 1";
            List<SqlParameter> List = new List<SqlParameter>();
            SqlParameter param = new SqlParameter("@Year", DbType.Int32);
       ...
       /* public static bool UpdateHBLSequence()
        ...
        public bool UpdateJobSequence(int MS)
        {
            int Id = 0;
            if (MS == 1)
            {
            ...
            String sql = @"UPDATE [dbo].[Numbering]
                                SET     [Sequence] = [Sequence] + 1
                                        ,LastUpdate = @Year
                                WHERE Id= 2"; // One Squence for All the job Type
            List<SqlParameter> List = new List<SqlParameter>();
        ...
        JB.DeliveryPlace,
            ...
            string sql = @"SELECT JB.JobNo,
	                      CONCAT(cust.[name],', ',cust.RegNo) AS Customer,
	                       ...
	                       JBP.[Name] AS ShiftRequest,
	                       --CM.[Name] AS ClearanceMode,
CONCAT(CM.[Name],'<br>',CLvehicle.[Name],'<br>', CLvehicle.RegNo) AS ClearanceMode,
	                       CP.[Name] AS ClearancePort,
	                       CS.[Name] AS ClearanceShift,
	                       CONCAT(CLparty.[name],', ',CLparty.RegNo) AS ClearanceParty,
	                       JB.AssignStaffMCS,
        ...
        JB.DeliveryPlace,
                    ...
                    LEFT JOIN ClearanceShift AS CS ON CS.Id = JB.ClearanceShiftId
                    LEFT JOIN Party AS CLparty ON CLparty.Id = JB.ClearancePartyId
LEFT JOIN ClearanceVehicle AS CLvehicle ON CLvehicle.Id = JB.ClearanceVehicleId -------------------------------------------
                    WHERE JB.DateRemoved IS NULL AND SP.ModeofShipmentId = 1 AND CAST(JB.ClearanceDate AS DATE)= @CLdate
                    --AND Convert(DATE,JB.ClearanceDate)='2023-07-31'
                    ORDER BY JB.ClearanceDate DESC";
        ...
        {
            Dictionary<Object, Object> dictionary = new Dictionary<Object, Object>();
            string sql = @"SELECT J.Id, J.JobNo,BL.ShipmentId,BL.Id AS BLid
                            FROM job AS J
            ...
            if (reader.Read())
            {
                ...
                dictionary.Add("lastBLId", reader["BLid"].ToString());
            }
            reader.Close();
            return dictionary;
        }
...
LEFT JOIN Employee AS EMP ON EMP.Id = JB.EmployeeId";
        ...
        public static Dictionary<object, object> GetJobbyClearanceStatus(int StatusId)
        {
...
LEFT JOIN Employee AS EMP ON EMP.Id = JB.EmployeeId
        ...
        public static Dictionary<object, object> GetNewJobs(int StatusId) // here new job status changed from Query including BL status Id 4,5,8,9 abd shipment statusId 1,2,3,4,5,6,7
        {
...
LEFT JOIN Employee AS EMP ON EMP.Id = JB.EmployeeId
        ...
        public static List<Dictionary<object, object>> GetPendingBLs()
        {
            List<Dictionary<object, object>> BLlist = new List<Dictionary<object, object>>();
            string sql = @"SELECT HBL.CustomerId,
                               HBL.Id,
	                           HBL.ShipmentId,
	                           HBL.HouseBL,
	                           P.Name,
	                           ...
	                           BLLS.BLStatusId
                        FROM HouseBL AS HBL
                        INNER JOIN BLLastStatus AS BLLS ON BLLS.BLId = HBL.Id AND BLLS.BLStatusId <> 7
                        INNER JOIN Party AS P ON P.Id = HBL.CustomerId
                        WHERE HBL.DateRemoved IS NULL AND HBL.HouseBL IS NOT NULL
                        ORDER BY HBL.CustomerId";
            List<SqlParameter> spList = new List<SqlParameter>();
            SqlDataReader reader = DBAccess.FetchResult(sql, spList, ConnectionString.DEFAULT);
            while (reader.Read())
            {
...
LEFT JOIN Party AS PTDA ON PTDA.Id = SP.DeliveryAgentId
            ...
            if (reader.Read())
            {
                ...
                dictionary.Add("EntryTypeId", reader["EntryTypeId"].ToString());
        ...
        public static bool MoveTheBL(int BLid, int CurrShipmentId, int NewShipmentId)
        {
            String sql = @"UPDATE HouseBL
                            SET ShipmentId = @NewShipmentId
	                            ,BLmovedBy = @EmployeeId
	                            ,BLmovedDate = GETDATE()
                            ...
                            UPDATE JobDocuments
                            SET ShipmentId = @NewShipmentId
                            WHERE HouseBLId = @BLId";
            List<SqlParameter> List = new List<SqlParameter>();

            SqlParameter param = new SqlParameter("@NewShipmentId", DbType.Int32);
            param.Value = NewShipmentId;
            List.Add(param);
            param = new SqlParameter("@PrevShipmentId", DbType.Int32);
            param.Value = CurrShipmentId;
            List.Add(param);
            param = new SqlParameter("@BLId", DbType.Int32);
            param.Value = BLid;
            List.Add(param);
            param = new SqlParameter("@EmployeeId", DbType.Int32);
            param.Value = Convert.ToInt32(Profile.GetProfile(HttpContext.Current.User.Identity.Name).GetPropertyValue("ACSUserId"));
...
,HBL.BLStateId
        ...
        public static List<Dictionary<object, object>> GetHouseBLItem(int SPId, string HBLno)
        {
            List<Dictionary<object, object>> HBLList = new List<Dictionary<object, object>>();
...
,HBL.BLStateId
							 ...
							 -- LEFT JOIN Party AS DOAgentP ON DOAgentP.Id = HBL.DOAgentId
							  ...
							  --INNER JOIN ContainerType AS CT ON CT.Id = HBL.ContainerTypeId
							 -- INNER JOIN TypeofPackage AS TP ON TP.Id = HBL.TypeofPackageId
							  WHERE HBL.DateRemoved IS NULL AND HBL.ShipmentId = @SPId AND HBL.HouseBL =@HouseBL";
...
CI.CMeasure
            ...
            while (reader.Read())
            {
               ...
               // containerInfo = reader["ContainerNo"].ToString() + "/" + reader["SealNo"].ToString() + "/" + reader["ContainerSizeType"].ToString() + "/" + reader["ContainerIndicator"].ToString() + "/" + reader["Package"].ToString() +"<br/>";
                containerInfo = reader["ContainerNo"].ToString() + "/" + reader["SealNo"].ToString() + "/" + reader["ContainerSizeType"].ToString() + "/" + reader["ContainerIndicator"].ToString() + "/" + reader["Package"].ToString() + "/" + reader["CWeight"].ToString() + "/" + reader["CMeasure"].ToString() + "<br/>";
    ...
    public class ContainerModel
    {
        ...
        public double CMeasure { get; set; }
